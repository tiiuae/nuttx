...
bool nxsched_switch_running(int cpu)
{
  ...
  rr = (rtcb->flags & TCB_FLAG_POLICY_MASK) == TCB_FLAG_SCHED_RR &&
    rtcb->timeslice <= 0;

  if (rr)
    {
      /* Change priority comparison below from > to >= */
...
bool nxsched_add_readytorun(FAR struct tcb_s *btcb)
{
   ...
   * try to run it some CPU if it was added to the head of the list
   */
      ...
      if (cpu < CONFIG_SMP_NCPUS)
        {
          ...
          if (tcb->sched_priority < btcb->sched_priority)
            {
              doswitch = nxsched_deliver_task(cpu, this_cpu());
            }
        }
    }
