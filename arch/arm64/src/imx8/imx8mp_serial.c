/****************************************************************************
 * arch/arm64/src/imx8/imx8_serial.c
 *
...
#  if defined(CONFIG_IMX8_UART1)
#    define TTYS0_DEV           g_uart1port /* UART1 is ttyS0 */
#    define UART1_ASSIGNED      1
#  elif defined(CONFIG_IMX8_UART2)
#    define TTYS0_DEV           g_uart2port /* UART2 is ttyS0 */
#    define UART2_ASSIGNED      1
#  elif defined(CONFIG_IMX8_UART3)
#    define TTYS0_DEV           g_uart3port /* UART3 is ttyS0 */
#    define UART3_ASSIGNED      1
#  elif defined(CONFIG_IMX8_UART4)
#    define TTYS0_DEV           g_uart4port /* UART4 is ttyS0 */
#    define UART4_ASSIGNED      1
#  endif
#endif
...
#if defined(CONFIG_IMX8_UART1) && !defined(UART1_ASSIGNED)
#  define TTYS1_DEV           g_uart1port /* UART1 is ttyS1 */
#  define UART1_ASSIGNED      1
#elif defined(CONFIG_IMX8_UART2) && !defined(UART2_ASSIGNED)
#  define TTYS1_DEV           g_uart2port /* UART2 is ttyS1 */
#  define UART2_ASSIGNED      1
#elif defined(CONFIG_IMX8_UART3) && !defined(UART3_ASSIGNED)
#  define TTYS1_DEV           g_uart3port /* UART3 is ttyS1 */
#  define UART3_ASSIGNED      1
#elif defined(CONFIG_IMX8_UART4) && !defined(UART4_ASSIGNED)
...
#if defined(CONFIG_IMX8_UART2) && !defined(UART2_ASSIGNED)
...
#if defined(CONFIG_IMX8_UART3) && !defined(UART3_ASSIGNED)
...
#if defined(CONFIG_IMX8_UART4) && !defined(UART4_ASSIGNED)
...
struct imx8_uart_s
{
  int      clock;       /* Peripheral clock as described in hardware/imx8_ccm.h */
...
static inline uint32_t imx8_serialin(struct imx8_uart_s *priv,
                                      uint32_t offset);
static inline void imx8_serialout(struct imx8_uart_s *priv,
                                   uint32_t offset,
                                   uint32_t value);
static inline void imx8_disableuartint(struct imx8_uart_s *priv,
                                        uint32_t *ucr1);
static inline void imx8_restoreuartint(struct imx8_uart_s *priv,
                                        uint32_t ucr1);
static inline void imx8_waittxready(struct imx8_uart_s *priv);
...
static int  imx8_interrupt(int irq, void *context, void *arg);
...
#ifdef CONFIG_IMX8_UART1
static struct imx8_uart_s g_uart1priv =
{
...
#ifdef CONFIG_IMX8_UART2
static struct imx8_uart_s g_uart2priv =
{
...
#ifdef CONFIG_IMX8_UART3
static struct imx8_uart_s g_uart3priv =
{
...
#ifdef CONFIG_IMX8_UART4
static struct imx8_uart_s g_uart4priv =
{
                                        ...
                                        uint32_t *ucr1)
{
  ...
  imx8_serialout(priv, UART_UCR1_OFFSET, priv->ucr1);
}

/****************************************************************************
 * Name: imx8_restoreuartint
 ****************************************************************************/
...
static inline void imx8_waittxready(struct imx8_uart_s *priv)
{
  ...
  for (tmp = 1000 ; tmp > 0 ; tmp--)
    {
      if ((imx8_serialin(priv, UART_UTS_OFFSET) & UART_UTS_TXFULL) == 0)
        {
          break;
        }
    }
...
static int imx8_setup(struct uart_dev_s *dev)
{
  struct imx8_uart_s *priv = (struct imx8_uart_s *)dev->priv;
  ...
  uint32_t clk = imx8_ccm_get_clock(priv->clock);
  ret = imx8_uart_configure(priv->uartbase, clk, &config);

  /* Initialize the UCR1 shadow register */

  priv->ucr1 = imx8_serialin(priv, UART_UCR1_OFFSET);
  return ret;
...
static void imx8_shutdown(struct uart_dev_s *dev)
{
  ...
  imx8_serialout(priv, UART_UCR1_OFFSET, 0);
  imx8_serialout(priv, UART_UCR2_OFFSET, 0);
  imx8_serialout(priv, UART_UCR3_OFFSET, 0);
  imx8_serialout(priv, UART_UCR4_OFFSET, 0);
}
...
static int imx8_attach(struct uart_dev_s *dev)
{
  ...
  ret = irq_attach(priv->irq, imx8_interrupt, dev);
  if (ret == OK)
    {
...
/****************************************************************************
 * Name: imx8_interrupt (and front-ends)
 *
 * Description:
...
static int imx8_interrupt(int irq, void *context, void *arg)
{
  ...
  struct imx8_uart_s *priv;
  uint32_t usr1;
  int passes = 0;

  DEBUGASSERT(dev != NULL && dev->priv != NULL);
  priv = (struct imx8_uart_s *)dev->priv;
  ...
  for (; ; )
    {
      ...
      usr1 = imx8_serialin(priv, UART_USR1_OFFSET);
      ...
      if (usr1 & UART_USR1_TRDY &&
         (imx8_serialin(priv, UART_UCR1_OFFSET) & UART_UCR1_TXEMPTYEN) != 0)
        {
          uart_xmitchars(dev);
        }
...
static int imx8_ioctl(struct file *filep, int cmd, unsigned long arg)
{
  ...
  switch (cmd)
    ...
    case TIOCSERGSTRUCT:
      {
         struct imx8_uart_s *user = (struct imx8_uart_s *)arg;
         if (!user)
           {
             ret = -EINVAL;
           }
           ...
           {
             memcpy(user, dev, sizeof(struct imx8_uart_s));
           }
       }
       break;
#endif
...
static int imx8_receive(struct uart_dev_s *dev, unsigned int *status)
{
  ...
  rxd0    = imx8_serialin(priv, UART_RXD_OFFSET);
  *status = rxd0;
...
static bool imx8_rxavailable(struct uart_dev_s *dev)
{
  ...
  return ((imx8_serialin(priv, UART_USR2_OFFSET) & UART_USR2_RDR) != 0);
}
...
{
  struct imx8_uart_s *priv = (struct imx8_uart_s *)dev->priv;
  imx8_serialout(priv, UART_TXD_OFFSET, (uint32_t)ch);
}
...
static bool imx8_txready(struct uart_dev_s *dev)
{
  ...
  return ((imx8_serialin(priv, UART_UTS_OFFSET) & UART_UTS_TXFULL) == 0);
}
...
static bool imx8_txempty(struct uart_dev_s *dev)
{
  ...
  return ((imx8_serialin(priv, UART_USR2_OFFSET) & UART_USR2_TXDC) != 0);
}
 ...
 *   that imx8_earlyserialinit was called previously.
 *
 ****************************************************************************/
...
{
#ifdef HAVE_UART_CONSOLE
  struct imx8_uart_s *priv = (struct imx8_uart_s *)CONSOLE_DEV.priv;
  uint32_t ier;
...
#  undef IMX8_CONSOLE_VBASE
#  if defined(CONFIG_UART1_SERIAL_CONSOLE)
#    define IMX8_CONSOLE MX8M_UART1
#  elif defined(CONFIG_UART2_SERIAL_CONSOLE)
#    define IMX8_CONSOLE MX8M_UART2
#  elif defined(CONFIG_UART3_SERIAL_CONSOLE)
#    define IMX8_CONSOLE MX8M_UART3
#  elif defined(CONFIG_UART4_SERIAL_CONSOLE)
#    define IMX8_CONSOLE MX8M_UART4
#  endif
...
#ifdef IMX8_CONSOLE
static inline void imx8_waittxready(void)
{
  int tmp;
  ...
  for (tmp = 1000 ; tmp > 0 ; tmp--)
    {
      ...
      if ((getreg32(IMX8_CONSOLE + UART_UTS_OFFSET) & UART_UTS_TXFULL) == 0)
        {
          break;
        }
    }
  ...
  if (ch == '\n')
    {
      ...
      putreg32((uint16_t)'\r', IMX8_CONSOLE + UART_TXD_OFFSET);
      imx8_waittxready();
    }

  putreg32((uint16_t)ch, IMX8_CONSOLE + UART_TXD_OFFSET);
#endif
