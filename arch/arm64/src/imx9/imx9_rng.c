...
/****************************************************************************
 * Name: imx9_rng_allzero
 *
 * Description:
 *   A block of zeros is what a silently failed ELE transfer leaves behind:
 *   the buffer is never written and the driver cannot tell that from a
 *   result.  Rejecting it costs nothing, since a genuine all-zero block has
 *   probability 2^-512, and it covers the first block, which the continuous
 *   test below cannot.
 *
 ****************************************************************************/

static bool imx9_rng_allzero(const uint8_t *buf, size_t len)
{
  size_t i;
...
static int imx9_rng_block(void)
{
  ...
  /* The ELE addresses the buffer physically and the parameter is 32 bits
   * wide, so a buffer the ELE cannot name has to fail here rather than have
   * its address truncated into someone else's memory.
   */

  if ((uintptr_t)g_rngbuf > UINT32_MAX)
    {
      _err("ERROR: buffer at %p is outside the ELE address range\n",
           g_rngbuf);
      return -EFAULT;
    }
  ...
  ret = imx9_ele_get_random((uint32_t)(uintptr_t)g_rngbuf,
                            sizeof(g_rngbuf));
  if (ret < 0)
    {
  ...
  if (imx9_rng_allzero(g_rngbuf, sizeof(g_rngbuf)))
    {
