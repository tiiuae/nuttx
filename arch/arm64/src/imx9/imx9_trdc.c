...
struct trdc_config_info trdc_boot_cfg_info[] =
{
  {
    0x49010000,
    trdc_boot_mbc_glbac, ARRAY_SIZE(trdc_boot_mbc_glbac),
    trdc_boot_ocram_mbc, ARRAY_SIZE(trdc_boot_ocram_mbc),
    trdc_n_mrc_glbac, ARRAY_SIZE(trdc_n_mrc_glbac),
    trdc_n_mrc, ARRAY_SIZE(trdc_n_mrc)
  },
};
     ...
     uint8_t pe, uint8_t pidm, uint8_t pid, bool lock)
{
  ...
  val = BIT(31) | ((pid & 0x3f) << 16)
    | ((pidm & 0x3f) << 8) | ((pe & 0x3) << 6) |
...
uint32_t trdc_mbc_blk_num(unsigned long trdc_reg, uint32_t mbc_x,
  uint32_t mem_x)
{
...
int trdc_mbc_set_control(unsigned long trdc_reg, uint32_t mbc_x,
...
int trdc_mrc_rgn_config(unsigned long trdc_reg,
...
static void trdc_mgr_fused_slot_setup
  (struct trdc_fused_module_info *fused_slot)
{
  uint32_t i;
  uint32_t val;
  uint32_t did;
...
int imx9_trdc_init(void)
{
  ...
  ret = imx9_release_rdc(0x7402);
  ret |= imx9_release_rdc(0x8202);
  ret |= imx9_release_rdc(0x7802);
  ret |= imx9_release_rdc(0x8602);
  ...
  trdc_setup(&trdc_boot_cfg_info[0]);
