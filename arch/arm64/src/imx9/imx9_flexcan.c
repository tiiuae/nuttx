...
static int imx9_transmit(struct imx9_driver_s *priv)
{
  ...
  if (can_id & CAN_EFF_FLAG)
    {
      cs |= CAN_MB_CS_IDE;
      mb->id = (can_id << CAN_MB_ID_ID_SHIFT) & CAN_MB_ID_ID_MASK;
    }
  else
    {
      mb->id = (can_id << CAN_MB_ID_ID_STD_SHIFT) & CAN_MB_ID_ID_STD_MASK;
    }

  cs |= (can_id & CAN_RTR_FLAG) ? CAN_MB_CS_RTR : 0;
  cs |= (((uint32_t)can_dlc) << CAN_MB_CS_DLC_SHIFT) & CAN_MB_CS_DLC_MASK;
  ...
  mb->cs = (cs | (CAN_TXMB_DATAORREMOTE << CAN_MB_CS_CODE_SHIFT));
  ...
  while ((f = flags) != 0)
    {
      ...
      uint32_t t = CAN_MB_CS_TIMESTAMP(rf->cs);
      while ((f &= ~(1 << mbj)) != 0)
        {
          mbj = arm64_lsb(f);
          struct mb_s *rf_next = flexcan_get_mb(priv, mbj);
          uint16_t t_next = CAN_MB_CS_TIMESTAMP(rf_next->cs);
...
#if 0
  uint32_t esr1 = getreg32(priv->base + IMX9_CAN_ESR1_OFFSET);
  _alert("ESR1: %x\n", esr1);
  _alert("ESR2: %x\n", getreg32(priv->base + IMX9_CAN_ESR2_OFFSET));
  _alert("IFLAG1: %x\n", getreg32(priv->base + IMX9_CAN_IFLAG1_OFFSET));
  _alert("ECR: %x\n", getreg32(priv->base + IMX9_CAN_ECR_OFFSET));

  /* ERRSR[HANCEIF], ERRSR[FANCEIF], and ERRSR[CEIF] */

  _alert("ERRSR: %x\n", getreg32(priv->base + IMX9_CAN_ERRSR_OFFSET));
  _alert("RERRAR: %x\n", getreg32(priv->base + IMX9_CAN_RERRAR_OFFSET));
  _alert("RERRDR: %x\n", getreg32(priv->base + IMX9_CAN_RERRDR_OFFSET));
  _alert("RERRSYNR: %x\n", getreg32(priv->base + IMX9_CAN_RERRSYNR_OFFSET));

  putreg32(esr1, priv->base + IMX9_CAN_ESR1_OFFSET);
  putreg32(0, priv->base + IMX9_CAN_ECR_OFFSET);
#endif
                                   ...
                                   uint32_t mbi)
{
  ...
  return (struct mb_s *)(priv->base + mb_offset);
}
...
static int imx9_init_eccram(struct imx9_driver_s *priv)
{
  ...
  /* Fill in the inactive RX buffers */
  ...
  for (mbi = 0; mbi < RXMBCOUNT; mbi++)
    {
      ...
      /* ID + Data fields until next mb */

      data = &mb->id;
      mb = flexcan_get_mb(priv, mbi + 1);
      while (data < (uint32_t *)mb && data < data_end)
        {
          *data++ = 0;
        }
    }

  /* Fill in the rest of the area as inactive TX buffers */

  mb = flexcan_get_mb(priv, mbi);
  while (data < (uint32_t *)(priv->base + IMX9_CAN_MB_END))
    {
  ...
  for (int i = 0; i < IMX9_CAN_N_RXIMR; i++)
    {
  ...
  /* Clear Tx_SMB, Rx_SMB0 and Rx_SMB1 - each size of 8-byte mb */
...
static int imx9_initialize(struct imx9_driver_s *priv)
{
  ...
  /* Clear and enable IFLAG1 interrupts (MB 0-20) */

  putreg32(IFLAG1_RX, priv->base + IMX9_CAN_IFLAG1_OFFSET);
  putreg32(IFLAG1_RX, priv->base + IMX9_CAN_IMASK1_OFFSET);

  /* Disable IFLAG2 & 3 interrupts */

  putreg32(0, priv->base + IMX9_CAN_IMASK2_OFFSET);
  putreg32(0, priv->base + IMX9_CAN_IMASK3_OFFSET);
