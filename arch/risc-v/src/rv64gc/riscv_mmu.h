...
static inline void mmu_invalidate_tlb_by_vaddr(uint64_t vaddr)
{
  __asm__ __volatile__
    (
      "sfence.vma x0, %0\n"
...
void mmu_ln_map_region(uint32_t ptlevel, uint64_t lnvaddr, uint64_t paddr,
                       uint64_t vaddr, size_t size, uint32_t mmuflags);

#endif /* ___ARCH_RISC_V_SRC_RV64GC_RISCV_MMU_H_ */
