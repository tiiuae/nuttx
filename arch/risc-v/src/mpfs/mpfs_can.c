...
static uint32_t mpfs_can_get_can_command_reg(mpfs_can_instance_t *priv);

static uint32_t mpfs_can_get_can_config_reg(mpfs_can_instance_t *priv);
#endif
...
#ifdef CONFIG_DEBUG_CAN_INFO
static bool mpfs_can_get_rx_gte96(mpfs_can_instance_t *priv);
#endif
...
#ifdef CONFIG_DEBUG_CAN_INFO
static bool mpfs_can_get_tx_gte96(mpfs_can_instance_t *priv);
#endif
                                       ...
                                       struct can_frame *cf)
{
  ...
  if (expect_true(!priv->filter.use_mask_filter
      && !priv->filter.use_range_filter)
      || (priv->filter.use_mask_filter
  ...
  for (i = 0; i < dlc; i++)
    {
      if (i < 4)
        {
          cf->data[i] = (uint8_t)((pmsg->data_high >> ((3 - i) * 8)) & 0xff);
        }
      else
        {
          cf->data[i] = (uint8_t)((pmsg->data_low >> ((7 - i) * 8)) & 0xff);
        }
    }
  ...
  while (CAN_VALID_MSG == mpfs_can_get_message(priv))
    {
      ...
      */
      ...
      /* Point the packet buffer back to the next Tx buffer that will be
        ...
        * to.  This is CAN_OK because devif_poll won't be called unless the
        * queue is not full.
        */
      ...
      for (i = 0; i < cf->can_dlc; i++)
        {
          if (i < 4)
            {
              pmsg->data_high |= (cf->data[i] << ((3 - i) * 8));
            }
          else
            {
              pmsg->data_low |= (cf->data[i] << ((7 - i) * 8));
            }
        }
    }
...
static uint32_t mpfs_can_msgid_to_canid(uint32_t id,
                                        ...
                                        bool rtr)
{
  id = (uint32_t)(ide ? id : (id >> MPFS_CAN_MSG_IDE_SHIFT)
...
static void mpfs_can_start(mpfs_can_instance_t *priv)
{
  ...
  mpfs_can_set_int_ebl(priv, MPFS_CAN_INT_ENABLE_INT_ENBL
                           ...
                           | MPFS_CAN_INT_ENABLE_ACK_ERR_INT_ENBL
                           | MPFS_CAN_INT_ENABLE_FORM_ERR_INT_ENBL
                           | MPFS_CAN_INT_ENABLE_CRC_ERR_INT_ENBL
                           | MPFS_CAN_INT_ENABLE_BUS_OFF_INT_ENBL
                           | MPFS_CAN_INT_STATUS_STUCK_AT_0);

  /* Intialize error status and counts */
  ...
  switch (cmd)
    ...
    case SIOCSCANBITRATE:
...

      {
        ...
        if (CAN_OK != mpfs_can_set_bitrate(priv, req->arbi_bitrate * 1000))
          {
