...
{
  return (uint32_t) (getreg64(MPFS_CLINT_MTIME) & 0xffffffff);
}
...
{
  ts->tv_sec  = elapsed / TICK_PER_SEC;
  elapsed    -= (uint64_t)ts->tv_sec * TICK_PER_SEC;
  ts->tv_nsec = elapsed * 1000000000 / TICK_PER_SEC;
}
