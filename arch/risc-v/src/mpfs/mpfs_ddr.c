...
#ifdef CONFIG_DEBUG_DDR_INFO
#  define ddrinfo       _info
#else
#  define ddrinfo       _none
#endif

#ifdef CONFIG_DEBUG_DDR_ERROR
#  define ddrerr         _err
#else
#  define ddrerr        _none
#endif
...
#define SIZE_OF_PATTERN_TEST            0x02000000
#define SIZE_OF_PATTERN_OFFSET          12
#define NO_PATTERN_IN_CACHE_READS       1

#define DDR_MODE_REG_VREF               0xc
...
#define MTC_TIMEOUT_ERROR               0x02

#define DEFAULT_RPC_166_VALUE           2

#define ONE_GB_MTC                      30
#define HALF_GB_MTC                     29
#define ONE_MB_MTC                      20

#define MSS_SCB_ACCESS_CONFIG           ((160ul << 8) | (0x80))
...
#define DDR_1067_MHZ                    1067000000
...
#define SW_TRAING_BCLK_SCLK_OFFSET      0x00000000
...
enum ddr_ss_status_e
{
    DDR_SETUP_DONE      = 0x01,
    DDR_SETUP_FAIL      = 0x02,
    DDR_SETUP_SUCCESS   = 0x04,
    DDR_SETUP_OFF_MODE  = 0x08,
    DDR_SETUP_RESET     = 0x10,
    DDR_SETUP_RESTART   = 0x20,
};
...
static void mpfs_ddr_manual_addcmd_training(struct mpfs_ddr_priv_s *priv)
{
  ...
  bclk_phase = ((priv->bclk_answer + SW_TRAING_BCLK_SCLK_OFFSET) &
                0x07) << 8;
  bclk90_phase = ((priv->bclk_answer + SW_TRAING_BCLK_SCLK_OFFSET + 2) &
                  0x07) << 11;
  ...
  /* SET Store DRV & VREF initial values (to be re-applied after
   * CA training)
   */
...
static int mpfs_training_verify(void)
{
  ...
  if (t_status != 0)
    {
      ddrerr("Training failed. Restarting!\n");
      return -EAGAIN;
    }
...
static int mpfs_training_write_calib_retry(struct mpfs_ddr_priv_s *priv)
{
  ...
  priv->write_latency++;
  if (priv->write_latency > MAX_LATENCY)
    {
      priv->write_latency = MIN_LATENCY;
      ddrerr("Write calib fail!\n");
      return -EIO;
    }
  else
    {
...
static int mpfs_ddr_test_32bit_nc(struct mpfs_ddr_priv_s *priv)
{
  ...
  if (error)
    {
      ddrerr("Found errors, memory is corrupt!\n");
      return -EIO;
    }
 ...
 *   references for the manufacturer's HSS code.
 *
 * Input Parameters:
 *   priv    - Instance of the ddr private state structure
 *
...
int mpfs_ddr_init(void)
{
  ...
  if (ddr_status == 0)
    {
      ddrinfo("DDR setup successfully\n");
    }
  else
    {
      ddrinfo("DDR setup returned error: %d\n", ddr_status);
      mpfs_ddr_fail(priv);
    }

  ddrinfo("Done ddr setup\n");
