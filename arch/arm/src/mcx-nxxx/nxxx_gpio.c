...
/****************************************************************************
 * Name: nxxx_gpio_configport
 ****************************************************************************/

static inline int nxxx_gpio_configport(gpio_pinset_t pinset)
{
  uint32_t port    = (pinset & GPIO_PORT_MASK) >> GPIO_PORT_SHIFT;
  uint32_t pin     = (pinset & GPIO_PIN_MASK) >> GPIO_PIN_SHIFT;
  uint32_t portcfg = pinset << GPIO_PORT_CFG_SHIFT;
  uint32_t regaddr = NXXX_PORT_PCR_BASE(port, pin);

  putreg32(portcfg, regaddr);
}
