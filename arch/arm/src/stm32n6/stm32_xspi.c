...
#ifndef STM32_XSPI2_KERNEL_FREQUENCY
#  error "STM32_XSPI2_KERNEL_FREQUENCY must be provided by the board"
#endif
...
static int xspi_iom_config(struct stm32_xspidev_s *priv,
                           const struct stm32_xspi_config_s *config)
{
  ...
  oldreg = getreg32(STM32_XSPIM_CR);
  port1 = (oldreg & XSPIM_CR_MODE) != 0 ? STM32_XSPI_IOM_PORT2 :
                                         STM32_XSPI_IOM_PORT1;
  port2 = ((oldreg & XSPIM_CR_MUXEN) != 0) !=
          ((oldreg & XSPIM_CR_MODE) != 0) ? STM32_XSPI_IOM_PORT1 :
                                           STM32_XSPI_IOM_PORT2;

  if ((oldreg & XSPIM_CR_CSSEL_OVR_EN) == 0)
    {
    ...
    {
      cs1 = (oldreg & XSPIM_CR_CSSEL_OVR_O1) != 0 ?
            STM32_XSPI_CS_NCS2 : STM32_XSPI_CS_NCS1;
      cs2 = (oldreg & XSPIM_CR_CSSEL_OVR_O2) != 0 ?
            STM32_XSPI_CS_NCS2 : STM32_XSPI_CS_NCS1;
    }

  clocks = getreg32(STM32_RCC_AHB5ENR);
  if ((clocks & RCC_AHB5ENR_XSPI1EN) != 0)
    {
      xspi1cr = getreg32(STM32_XSPI1_CR);
    }

  if ((clocks & RCC_AHB5ENR_XSPI2EN) != 0)
    {
      xspi2cr = getreg32(STM32_XSPI2_CR);
    }

  /* The boot ROM leaves the flash on XSPI1 through swapped mode.  Moving
   * XSPI2 to port 2 must return XSPI1 to port 1 rather than accidentally
   * turning the swapped configuration into multiplexed mode.
   */

  handoff = priv->intf == 2 && config->iomport == STM32_XSPI_IOM_PORT2 &&
            (oldreg & (XSPIM_CR_MUXEN | XSPIM_CR_MODE)) == XSPIM_CR_MODE;
  ...
  if ((xspi1cr & XSPI_CR_EN) != 0)
    {
      ret = xspi_wait(&g_xspi1dev, XSPI_SR_BUSY, 0);
      if (ret < 0)
        {
          return ret;
        }
  ...
  if ((xspi2cr & XSPI_CR_EN) != 0)
    {
      ret = xspi_wait(&g_xspi2dev, XSPI_SR_BUSY, 0);
      if (ret < 0)
        {
          if ((xspi1cr & XSPI_CR_EN) != 0)
            {
  ...
  if ((clocks & RCC_AHB5ENR_XSPI1EN) != 0 && !handoff)
    {
      putreg32(xspi1cr, STM32_XSPI1_CR);
    }
...
static int xspi_hw_initialize(struct stm32_xspidev_s *priv)
{
  if (priv->intf == 1)
    {
      putreg32(RCC_AHB5ENR_XSPI1EN | RCC_AHB5ENR_XSPIMEN,
               STM32_RCC_AHB5ENSR);
      putreg32(RCC_AHB5LPENR_XSPI1LPEN | RCC_AHB5LPENR_XSPIMLPEN,
                                  ...
                                  uint32_t frequency)
{
  ...
  if (priv->intf == 2)
    {
      kernel_frequency = STM32_XSPI2_KERNEL_FREQUENCY;
    }
#ifdef STM32_XSPI1_KERNEL_FREQUENCY
  else
    {
      kernel_frequency = STM32_XSPI1_KERNEL_FREQUENCY;
    }
#else
  else
    {
  ...
  prescaler = (kernel_frequency + frequency - 1) / frequency;
  if (prescaler < 1)
    {
      prescaler = 1;
    }
  ...
  return kernel_frequency / prescaler;
}
                          ...
                          const struct stm32_xspi_config_s *config)
{
  ...
  ret = xspi_iom_config(priv, config);
  if (ret < 0)
    {
  ...
  priv->data_strobe  = config->data_strobe;
  cr &= ~(XSPI_CR_FTHRES_MASK | XSPI_CR_FMODE_MASK);

  /* Keep the controller's CSSEL output consistent with the XSPIM override.
   * The boot ROM may leave CSSEL selecting NCS2; XSPIM then ignores every
   * transaction when the board overrides this controller to NCS1.
   */

  if (config->cs_override != STM32_XSPI_CS_DISABLED)
    {
      cr &= ~XSPI_CR_CSSEL;
      if (config->cs_override == STM32_XSPI_CS_NCS2)
        {
          cr |= XSPI_CR_CSSEL;
        }
    }
                        ...
                        struct qspi_cmdinfo_s *cmdinfo)
{
  ...
  cmd.data_strobe       = priv->data_strobe;
