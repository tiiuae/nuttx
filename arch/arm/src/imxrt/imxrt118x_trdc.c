...
/* Packed ELE TRDC release IDs.  The ELE RELEASE_RDC payload is
 * (RdcID << 8) | CoreID, so parameterize by target core ID
 * (ELE_CORE_CM33_ID or ELE_CORE_CM7_ID).
 */

#define TRDC_AON(core)      ((ELE_TRDC_AON_ID    << 8) | ((core) & 0xff))
#define TRDC_WAKEUP(core)   ((ELE_TRDC_WAKEUP_ID << 8) | ((core) & 0xff))
#define TRDC_MEDIA(core)    ((ELE_TRDC_MEGA_ID   << 8) | ((core) & 0xff))
...
void imxrt118x_trdc_config(void)
{
  ...
  for (i = 0; i < nitems(trdc_gpio); i++)
    {
      mmio_write_32(trdc_gpio[i].address, trdc_gpio[i].value);
    }

  /* Apply board MDA settings. */

  for (i = 0; i < nitems(trdc_mda); i++)
    {
      trdc_mda_setup(&trdc_mda[i]);
    }
...
int imxrt118x_trdc_init(void)
{
  ...
  /* TRDC ordering constraint (see the TRDC_*() macros above): all cores'
   * AON and MEGA must be released before any core's WAKEUP.
   */

  ret  = imxrt118x_ele_release_rdc(TRDC_AON(ELE_CORE_CM33_ID));
  ret |= imxrt118x_ele_release_rdc(TRDC_AON(ELE_CORE_CM7_ID));
  ret |= imxrt118x_ele_release_rdc(TRDC_MEDIA(ELE_CORE_CM33_ID));
  ret |= imxrt118x_ele_release_rdc(TRDC_MEDIA(ELE_CORE_CM7_ID));
  ret |= imxrt118x_ele_release_rdc(TRDC_WAKEUP(ELE_CORE_CM33_ID));
  ret |= imxrt118x_ele_release_rdc(TRDC_WAKEUP(ELE_CORE_CM7_ID));
